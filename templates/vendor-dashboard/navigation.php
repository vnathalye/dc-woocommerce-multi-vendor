            ...
            <?php foreach ($nav_items as $key => $item): ?>
                <?php if (current_user_can($item['capability']) || $item['capability'] === true): ?>
                    <li class="<?php if(!empty($item['submenu'])){ echo 'hasmenu';} ?>">
                        ...
                        <a href="<?php echo esc_url($item['url']); ?>" target="<?php echo $item['link_target'] ?>" data-menu_item="<?php echo $key ?>" class="<?php echo wcmp_get_vendor_dashboard_nav_item_css_class($key, $force_active); ?>">
                        ...
                        <ul class="submenu" <?php if(!array_key_exists($WCMp->endpoints->get_current_endpoint(), $item['submenu'])){ echo 'style="display:none"'; } ?>>
                                <?php foreach ($item['submenu'] as $submenukey => $submenu): ?>
                                    <?php if(current_user_can($submenu['capability']) || $submenu['capability'] === true): ?>
                                        <li class="<?php echo wcmp_get_vendor_dashboard_nav_item_css_class($submenukey); ?>">
                                            <a href="<?php echo esc_url($submenu['url']); ?>" target="<?php echo $item['link_target'] ?>">- <?php echo esc_html($submenu['label']); ?></a>
                                        </li>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                            </ul>
