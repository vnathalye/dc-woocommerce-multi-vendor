...
class WCMp_Settings_Gneral_Policies {
    ...
    public function wcmp_general_policies_settings_sanitize($input) {
        ...
        if (!$hasError) {
            add_settings_error(
                    "wcmp_{$this->tab}_{$this->subsection}_settings_name", esc_attr("wcmp_{$this->tab}_{$this->subsection}_settings_admin_updated"), __('Policies Settings Updated', $WCMp->text_domain), 'updated'
            );
        }
