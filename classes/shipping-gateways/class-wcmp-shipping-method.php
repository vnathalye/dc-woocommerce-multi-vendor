...
class WCMP_Vendor_Shipping_Method extends WC_Shipping_Method {
    ...
    */
    public static function free_shipping_is_available( $package, $method ) {
        ...
        return apply_filters( 'wcmp_shipping_free_shipping_is_available', $has_met_min_amount, $package, $method );
    }
    ...
    */
    public function is_available( $package ) {
        ...
        $locations = WCMP_Shipping_Zone::get_locations( $zone->get_id(), $vendor_id );
    ...
    */
    public function get_method_rate_id( $method ) {
        return apply_filters( 'wcmp_get_vendor_shipping_method_id', $method['id'] . ':' . $method['instance_id'] );
    }

    /* @not used */
    public static function is_shipping_enabled_for_seller( $vendor_id ) {
        $vendor_shipping_details = get_user_meta( $vendor_id, '_wcmp_shipping', true );
        if( !empty($vendor_shipping_details) ) {
            $enabled = $vendor_shipping_details['_wcmp_user_shipping_enable'];
            $type = $vendor_shipping_details['_wcmp_user_shipping_type'];
