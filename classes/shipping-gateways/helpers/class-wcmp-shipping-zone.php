...
class WCMP_Shipping_Zone {
    public static function get_zones() {
        ...
        foreach ( $raw_zones as $raw_zone ) {
            ...
            if ( in_array( 'wcmp_vendor_shipping', $methods_id ) ) {
    ...
    public static function get_zone( $zone_id ) {
        ...
        if ( in_array( 'wcmp_vendor_shipping', $methods_ids ) ) {
    ...
    public static function delete_shipping_methods( $data ) {
        ...
        $result = $wpdb->query( $wpdb->prepare( "DELETE FROM {$table_name} WHERE zone_id=%d AND vendor_id=%d AND instance_id=%d", $data['zone_id'], apply_filters( 'wcmp_current_vendor_id', get_current_user_id() ), $data['instance_id'] ) );
    ...
    public static function update_shipping_method( $args ) {
        ...
        $data = array(
            ...
            'vendor_id' => empty( $args['vendor_id'] ) ? apply_filters( 'wcmp_current_vendor_id', get_current_user_id() ) : $args['vendor_id'],
