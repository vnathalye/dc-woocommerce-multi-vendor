	...
	<div class="wcmp_main_menu">
		<ul>
			...
			<li><a <?php if($selected_item == "dashboard") { echo 'class="active"'; } ?> data-menu_item="dashboard" href="<?php echo isset($pages['vendor_dashboard']) ? get_permalink($pages['vendor_dashboard']) : ''; ?>" data-menu_item="dashboard" ><i class="icon_stand ic1"> </i> <span class="writtings"><?php _e( 'Dashboard', $WCMp->text_domain ); ?></span></a></li>
				...
				<ul class="submenu" <?php if(!in_array($selected_item, apply_filters('wcmp_store_settings_sub_menu_options',array('shop_front', 'policies', 'billing', 'shipping')))) { ?> style="display:none;"<?php } ?>>
					...
					<?php $is_show_shipping_tab = apply_filters('is_wcmp_show_shipping_tab', true); ?>
                                        <?php if(isset($wcmp_payment_settings_name['give_shipping']) && get_option('woocommerce_calc_shipping') != 'no') { if(empty($_vendor_give_shipping) && $is_show_shipping_tab) {?>
			...
			<?php if($WCMp->vendor_caps->vendor_capabilities_settings('is_submit_product') && get_user_meta($vendor->id, '_vendor_submit_product' ,true) && $is_show_product_tab) { ?>
					<li class="hasmenu"><a <?php if(in_array($selected_item, array('product_manager', 'add_product_manager', 'pending_product_manager'))) { echo 'class="active"'; } ?>  data-menu_item="product_manager" href="<?php if(class_exists('WCMp_Frontend_Product_Manager')) { echo apply_filters('wcmp_vendor_submit_product_tab_link', '#'); } else { echo apply_filters('wcmp_vendor_submit_product', admin_url( 'edit.php?post_type=product' )); } ?>"><span class="icon_stand ic3 shop_url"> </span> <span class="writtings"><?php _e( 'Product Manager', $WCMp->text_domain ); ?></span></a>
