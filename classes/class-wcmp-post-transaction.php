  ...
  function register_post_type() {
		...
		if(!empty($transaction_data)) {
			foreach($transaction_data as $vendor_id => $transaction_detail) {
				...
				if($trans_id) {
					...
					if(!isset($transaction_detail['transfer_charge'])) $transaction_detail['transfer_charge'] = 0;
					...
					if($paypal_response) update_post_meta($trans_id, 'paypal_response', $paypal_response);
	...
	function get_transaction_item_details($transaction_id) {
		...
		foreach($commissions as $commission_id => $order_id ) {
			...
			if(!is_array($commission_products)) $commission_products = array($commission_products);
			if(!empty($commission_products)) {
				$title = '';
				foreach($commission_products as $commission_product) {
					if( function_exists( 'get_product' ) ) {
						$product = get_product( $commission_product );
					} else {
			...
			$commission_details['body'][$commission_id][]['Order'] = '<a href="'.esc_url( add_query_arg( array( 'order_id' => $order_id ), get_permalink($vendor_detail_page))).'" target="_blank">#'.$order_id.'</a>';
	...
	function get_transactions($vendor_term_id = false, $start_date= false, $end_date = false, $transaction_status = false, $offset = false,  $no_of = false) {
		...
		if(!$no_of) $no_of = -1;
		if(!$transaction_status) $transaction_status = 'any';
		...
		if($transactions) {
			foreach($transactions as $transaction_key => $transaction) {
				...
				if($transaction->post_status  == 'wcmp_completed') $transaction_details[$transaction->ID]['status'] = __('Completed',  $WCMp->text_domain);
				else if($transaction->post_status  == 'wcmp_processing') $transaction_details[$transaction->ID]['status'] = __('Processing',  $WCMp->text_domain);
				...
				if($mode == 'paypal_masspay') $transaction_details[$transaction->ID]['mode'] = __('PayPal',  $WCMp->text_domain);
				else if($mode == 'direct_bank') $transaction_details[$transaction->ID]['mode'] = __('Direct Bank Transfer',  $WCMp->text_domain);
			}
		}
	...
	function create_transactions($commission_ids) {
		...
		if(!empty($commission_ids)) {
      	...
      	if(!isset($transaction_datas[$vendor_id]['amount'])) $transaction_datas[$vendor_id]['amount'] =  $vendor_due; 
      	else $transaction_datas[$vendor_id]['amount'] +=  $vendor_due;
			}
