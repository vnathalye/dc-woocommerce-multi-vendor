...
if (!class_exists('WCMp_Frontend_WP_Fields')) {

    class WCMp_Frontend_WP_Fields {
        ...
        public function text_input($field) {
            ...
            printf(
                    '<input type="%s" id="%s" name="%s" class="form-control %s" value="%s" placeholder="%s" %s %s />', $field['type'], esc_attr($field['id']), esc_attr($field['name']), esc_attr($field['class']), esc_attr($this->string_wpml('' . $field['value'] . '')), esc_attr($this->string_wpml('' . $field['placeholder'] . '')), implode(' ', $custom_attributes), implode(' ', $attributes)
            );
        ...
        function textarea_input($field) {
            ...
            printf(
                    '<textarea id="%s" name="%s" class="form-control %s" placeholder="%s" rows="%s" cols="%s" %s>%s</textarea>', esc_attr($field['id']), esc_attr($field['name']), esc_attr($field['class']), esc_attr($field['placeholder']), absint($field['rows']), absint($field['cols']), implode(' ', $custom_attributes), esc_textarea($this->string_wpml($field['value']))
            );
        ...
        public function select_input($field) {
            ...
            printf(
                    '<select id="%s" name="%s" class="form-control %s" %s %s>%s</select>', esc_attr($field['id']), esc_attr($field['name']), esc_attr($field['class']), implode(' ', $custom_attributes), implode(' ', $attributes), $options
            );
        ...
        public function timezone_input($field) {
            ...
            printf(
                    '<select id="%s" name="%s" class="form-control %s" %s />%s</select>', esc_attr($field['id']), esc_attr($field['name']), esc_attr($field['class']), implode(' ', $custom_attributes), $options
            );
        ...
        public function multi_input($field) {
            ...
            printf(
                    '<div id="%s" class="%s multi_input_holder" data-name="%s" data-length="%s" %s>', $field['id'], $field['class'], $field['name'], count($field['value']), implode(' ', $custom_attributes)
            );
                ...
                for ($blockCount = 0; $blockCount < $eleCount; $blockCount++) {
                    ...
                    foreach ($field['options'] as $optionKey => $optionField) {
                        ...
                        if ($optionField['type'] == 'select') {
                            if (isset($field['value']) && isset($field['value'][$blockCount]) && isset($field['value'][$blockCount]['option_values']))
                                $optionField['options'] = $field['value'][$blockCount]['option_values'];
                        }
...
//            }
            ...
            printf('<div class="form-group %s">', $field['wrapper_class']);
...
//                }
                ...
                printf(
                        '<label class="control-label col-md-3 col-sm-3 %s" for="%s">%s ', $field['label_class'], $field['id'], $field['label']
                );
...
//                }
            ...
            printf('<div class=" col-md-6 col-sm-9">');
...
//            }
            ...
            if (isset($field['desc']) && !empty($field['desc'])) {
                ...
                if (!isset($field['desc_class']))
                    $field['desc_class'] = '';

                printf(
                        '<p class="description %s">%s</p>', wp_kses_post($field['desc_class']), wp_kses_post($field['desc'])
                );
...
//            }
        ...
        public function wcmp_generate_form_field($fields, $common_attrs = array()) {
            if (!empty($fields)) {
