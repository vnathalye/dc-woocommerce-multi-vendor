...
<div class="wcmp_main_holder toside_fix">
	<div class="wcmp_headding1">
		<ul>
			...
			<li class="next"> > </li>
			<li> <?php _e( 'Withdrawals', $WCMp->text_domain );?></li>
		</ul>
		<div class="clear"></div>
	</div>
	<?php
	if($WCMp->vendor_caps->payment_cap['commission_threshold']) $get_vendor_thresold = $WCMp->vendor_caps->payment_cap['commission_threshold'];
	else $get_vendor_thresold = 0;
	?>
	...
	<form name="get_paid_form" method="post">
		<div class="wcmp_table_holder">
			<table class="get_paid_orders" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<?php $WCMp->template->get_template('shortcode/vendor_withdrawal_items.php', array('vendor' => $vendor, 'commissions' => $commissions )); ?>
			</table>
		</div>
