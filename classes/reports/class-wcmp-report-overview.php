...
class WCMp_Report_Overview extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		for( $date = $start_date; $date <= strtotime( '+1 day', $end_date); $date = strtotime( '+1 day', $date ) ) {
			...
			if ( $order_count > 0 ) $avg_sales = $sales / $order_count;
			else $avg_sales = 0;
		...
		if ( $total_order_count > 0 ) $total_avg_sales = $total_sales / $total_order_count;
		else $total_avg_sales = 0;
	...
	public function get_main_chart() {
		...
		$chart_data = json_encode( array(
			...
			'vendor_total_earned'    => array_map( array( $this, 'round_chart_totals' ), array_values( $vendor_total_earned ) ),
		) );
		?>
