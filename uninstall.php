...
/**
 * WC Marketplace Uninstall
 *
 * Uninstalling WC Marketplace deletes user roles, pages, tables, and options.
 *
 ...
 */
if (defined('WCMP_REMOVE_ALL_DATA') && true === WCMP_REMOVE_ALL_DATA) {
    // Roles + caps.
    include_once( dirname(__FILE__) . '/includes/wcmp-core-functions.php' );
    remove_role('dc_vendor');
    remove_role('dc_pending_vendor');
    remove_role('dc_rejected_vendor');
    // Pages.
    wp_trash_post(wcmp_vendor_dashboard_page_id());
    wp_trash_post(wcmp_vendor_registration_page_id());
